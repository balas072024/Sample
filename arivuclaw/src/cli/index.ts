...
async function startGateway(): Promise<void> {
  ...
  const CHANNELS_REQUIRING_TOKEN: Record<string, string> = {
  ...
  const dashPort = Number((config.gateway as any).dashboardPort) || 6800;
  try {
  ...
  const attemptRestart = async (source: string) => {
    if (isRestarting) {
    ...
    if (restartAttempts < MAX_RESTART_ATTEMPTS) {
      ...
      try { await gateway.shutdown(); } catch { /* best effort */ }
...
function validateProviderKey(config: ArivuClawConfig): void {
  ...
  if (!envVar) return; // ollama, custom, neural-brain don't need keys
