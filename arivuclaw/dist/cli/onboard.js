...
exports.runOnboardWizard = runOnboardWizard;
...
`);
    try {
        ...
        if (telegramToken)
            console.log(`  Telegram:  ***${telegramToken.slice(-6)}`);
        if (discordToken)
            console.log(`  Discord:   ***${discordToken.slice(-6)}`);
        ...
        if (apiKey)
            envLines.push(`${provider.envVar}=${apiKey}`);
        if (telegramToken)
