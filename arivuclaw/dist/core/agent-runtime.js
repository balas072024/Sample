...
const log = logger_1.Logger.create("agent-runtime");
...
class AgentRuntime {
    ...
    sandboxExecutor;
    maxToolRounds = 10;
    constructor(config, provider, memoryStore, skillRegistry) {
        this.config = config;
        this.provider = provider;
        this.memoryStore = memoryStore;
        this.skillRegistry = skillRegistry;
        this.securityGuard = new guard_1.SecurityGuard(config.security);
        this.sandboxExecutor = new sandbox_1.SandboxExecutor(config.security);
    }
    async processMessage(session, message, user) {
    ...
    async enrichWithMemory(session, message, user) {
        const parts = [];
    ...
    // ─── Skill Selection ─────────────────────────────────────────────
    async selectSkills(message, session) {
    ...
    // ─── Tool Execution ──────────────────────────────────────────────
    async executeToolCalls(toolCalls, session, user) {
        const results = [];
    ...
    // ─── Prompt Building ─────────────────────────────────────────────
    buildSystemPrompt(user, memoryContext, activeSkills) {
    ...
    buildConversation(session, currentMessage) {
    ...
    formatAssistantWithToolCalls(response) {
