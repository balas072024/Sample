...
const log = logger_1.Logger.create("system-tools");
class SystemTools {
    // ─── System Information ──────────────────────────────────────────
    static getFullSystemInfo() {
        return {
    ...
    static detectPackageManager() {
        ...
        for (const pm of managers) {
            try {
                (0, child_process_1.execSync)(`which ${pm}`, { stdio: "ignore" });
                return pm;
            }
            catch {
                continue;
    ...
    static installPackage(packageName, manager) {
    ...
    // ─── Service Management ──────────────────────────────────────────
    static manageService(service, action) {
        return this.run(`sudo systemctl ${action} ${service}`);
    }
    // ─── Clipboard ───────────────────────────────────────────────────
    static clipboardRead() {
    ...
    // ─── Screenshot ──────────────────────────────────────────────────
    static takeScreenshot(outputPath, delay = 0) {
        const platform = os.platform();
        switch (platform) {
            case "linux":
    ...
    // ─── Text-to-Speech ──────────────────────────────────────────────
    static textToSpeech(text, outputPath) {
        const platform = os.platform();
        const escaped = text.replace(/'/g, "'\\''");
        if (outputPath) {
            // Save to file
    ...
    // ─── Git ─────────────────────────────────────────────────────────
    static gitStatus(repoPath = ".") {
        return this.run(`cd '${repoPath}' && git status`);
    }
    static gitCommit(message, files, repoPath = ".") {
        let cmd = `cd '${repoPath}' && `;
    ...
    // ─── OCR ─────────────────────────────────────────────────────────
    static ocrImage(imagePath, language = "eng") {
        return this.run(`tesseract '${imagePath}' stdout -l ${language} 2>/dev/null`);
    }
    // ─── Helper ──────────────────────────────────────────────────────
    static run(command, timeout = 30_000) {
        try {
